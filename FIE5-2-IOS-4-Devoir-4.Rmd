...
---
#groupe: "Rahma Ben Younes - Léa Doublet"
...
```{r}
# Creation fichiers données acceuil

library(dplyr)
library(lubridate)
library(writexl)

# Données utilisées pour la simulation de l'accueil
data_accueil <- patients

# Heure de début de la journée simulée
heure_debut <- as.POSIXct(
  "2015-11-12 08:00:00",
  tz = "Europe/Paris"
)

# Extraction des arrivées à l'accueil
arrivees_accueil <- data_accueil %>%
  ...
  summarise(
    heure_arrivee = min(
      as.POSIXct(DATETIME_BEGIN, tz = "Europe/Paris")
    ),
    .groups = "drop"
  ) %>%
  
  arrange(heure_arrivee) %>%
...
# Export des arrivées pour AnyLogic
write_xlsx(
  arrivees_accueil,
  "local_data/fluxAccueil.xlsx"
)

write.csv2(
  arrivees_accueil,
  "local_data/fluxAccueil.csv",
  row.names = FALSE
)

# Paramètres Gamma du service accueil
params_gamma_accueil <- resultats_gamma %>%
  filter(ACTIVITY_MACRO == "Accueil")

write.csv2(
  params_gamma_accueil,
  "local_data/Gamma_accueil.csv",
  row.names = FALSE
)

# Vérification
print(arrivees_accueil)
print(params_gamma_accueil)
```
## 1- Arrivées réelles avec temps de service exponentiels:
Ce premier scénario utilise les arrivées réelles extraites des données hospitalières. Les patients sont générés dans AnyLogic à partir des horaires observés dans la base de données. Les temps de service sont modélisés par une loi exponentielle afin de représenter des durées de traitement aléatoires simples.

```{r}
knitr::include_graphics("Anylogic_images/Q2_param_reel.png")


```

```{r}
knitr::include_graphics("Anylogic_images/Q2_param_arriveesReels.png")
```

```{r}
knitr::include_graphics("Anylogic_images/Q2_Reel.png")
```

## 2. Arrivées de Poisson homogène avec temps exponentiels

Dans ce scénario, les arrivées suivent un processus de Poisson homogène avec un taux λ constant. Plusieurs valeurs de λ ont été testées afin d’observer l’évolution du niveau d’occupation et les phénomènes de saturation.


```{r}
knitr::include_graphics("Anylogic_images/Q2_param_arriveesPoissonsHomogene.png")
```

```{r}
knitr::include_graphics("Anylogic_images/Q2_poissonHomogene.png")
```



## 3. Arrivées de Poisson non homogène avec temps exponentiels

Les arrivées sont ici modélisées par un processus de Poisson non homogène. Le taux d’arrivée varie selon les périodes de la journée afin de reproduire des variations d’activité plus réalistes.

```{r}
knitr::include_graphics("Anylogic_images/Q2_param_poissonNonHomogene.png")
```
```{r}
knitr::include_graphics("Anylogic_images/Q2_PoissonNonHomogene.png")
```



## 4. Temps de service Gamma

Les temps de service exponentiels ont ensuite été remplacés par des lois Gamma ajustées à partir des données réelles sous R afin d’obtenir une représentation plus réaliste des durées de traitement.

```{r}
knitr::include_graphics("Anylogic_images/Q2_param_Gamma.png")
```

```{r}
knitr::include_graphics("Anylogic_images/Q2_Gamma.png")
```
...
# Combinaisons avec au moins 5 observations
combos <- ops_valid %>%
  group_by(ACTIVITY_MACRO, ACTOR_TYPE, PRIO) %>%
  summarise(n = n(), .groups = "drop") %>%
  filter(n >= 5)
...
Le service d’urologie a été modélisé sous AnyLogic à partir des circuits patients observés dans les données de la question 1. Les patients arrivent d’abord au service d’accueil avant d’être orientés vers une consultation ou un examen selon leur parcours. Après la consultation, les patients rejoignent directement le service de sortie. Après un examen, une orientation supplémentaire est réalisée avant la sortie du système. Ce modèle permet de représenter les principaux flux patients ainsi que l’utilisation des ressources du service au cours de la journée.

Le but est de :

- simuler le service d’urologie avec des arrivées de Poisson non homogènes
- faire varier le nombre d’agents selon l’heure
- trouver la configuration qui :
      * utilise le moins d’agents possible
      * sans saturation
      * avec tous les patients terminés avant 20h.


```{r}
knitr::include_graphics("Anylogic_images/Q3_ModèleAnylogic.png")
```

## 2. Tests des configurations:

Plusieurs configurations de planning ont été testées afin d’identifier le nombre minimal d’agents nécessaire pour assurer le bon fonctionnement du service. Les simulations permettent de comparer l’impact des ressources sur les temps d’attente, les temps de prise en charge et la saturation du système.
...
tests <- data.frame(
  ...
  Accueil_AM = c(1, 2, 2, 2),
  Accueil_PM = c(1, 1, 1, 1),
  
  Consultation_AM = c(1, 2, 3, 5),
  Consultation_PM = c(1, 1, 2, 4),
...
knitr::kable(tests,
             caption = "Configurations de planning testées pour le service d’urologie")
```
...
```{r}
knitr::include_graphics("Anylogic_images/Q3_test1.png")
```

=> La configuration avec un seul agent par service entraîne une saturation du système. Le dernier départ dépasse largement 20h, ce qui montre que les ressources sont insuffisantes pour absorber le flux de patients.

### b. Résultats test 2:


```{r}
knitr::include_graphics("Anylogic_images/Q3_test2.png")

```
=> L’augmentation du nombre d’agents permet de réduire le temps moyen passé dans le système ainsi que le niveau de saturation. Cependant, le dernier départ reste supérieur à 20h, ce qui indique que la configuration reste insuffisante pour absorber correctement le flux de patients.
...
```{r}
knitr::include_graphics("Anylogic_images/Q3_test3.png")

```

=> L’augmentation du nombre d’agents en consultation permet de réduire fortement le temps moyen passé dans le système. Cependant, malgré cette amélioration, certains patients terminent encore leur prise en charge après 20h. Le système reste donc légèrement saturé.

### d. Résultats test 4:


```{r}
knitr::include_graphics("Anylogic_images/Q3_test4.png")

```

=> L’augmentation du nombre d’agents en consultation permet de supprimer la saturation du système. L’ensemble des patients est correctement pris en charge et sort du système à la fin de la simulation.

# 4. Conclusion:

Plusieurs configurations de planning ont été testées sur les deux tranches horaires [8h ; 14h] et [14h ; 20h] en faisant varier le nombre d’agents affectés aux différents services.

Les simulations ont montré qu’une configuration avec un nombre insuffisant d’agents entraîne une saturation du système, une augmentation importante du temps moyen passé dans le système ainsi qu’un retard de prise en charge des patients. À l’inverse, l’augmentation progressive des ressources, principalement au niveau des consultations, permet d’améliorer les performances du système et de réduire les files d’attente.

La configuration retenue correspond à :

- 2 agents à l’accueil le matin et 1 l’après-midi
- 5 agents en consultation le matin et 4 l’après-midi
- 2 agents en examen le matin et 1 l’après-midi.

Cette configuration permet de traiter l’ensemble des patients sans saturation du système tout en limitant le nombre moyen d’agents mobilisés sur la journée.
